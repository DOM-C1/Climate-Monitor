"""Allow user to sign up, log in, and edit preferences."""
...
if __name__ == "__main__":
                ...
                if submit_button and email and password:
                    try:
                        ...
                        response = requests.post(
                            LOGIN_URL, json=data, headers=HEADERS, timeout=10)
    ...
    if st.session_state.get('is_logged_in'):
        ...
        response = requests.post(
            DETAILS_URL, headers=HEADERS, json=data, timeout=10)
        if response.status_code == 200:
            ...
            if st.button('Submit Changes'):
                ...
                for location in df['loc_name'].unique():
                    location_data = df[df['loc_name'] == location]
                    user_loc_id = location_data['user_location_id'].tolist()[0]
                    ...
                    if alert_value:
                        changes['alerts'].append(
                            {'id': user_loc_id, 'value': alert_value})
                    if report_value:
                        changes['reports'].append(
                            {'id': user_loc_id, 'value': report_value})
                response = requests.post(
                    LOC_URL, json=changes, headers=HEADERS, timeout=10)
                if response.status_code == 200:
        ...
        with st.popover('Delete Acount'):
            if st.button('Are you sure you want to delete your account?'):
                data = {'email': st.session_state['email'],
                        'password': st.session_state['hash_password']}
                response = requests.post(
                    DEL_URL, json=data, headers=HEADERS, timeout=10)
                if response.status_code == 200:
