...
def get_weather_details_for_week(latitude: int, longitude: int) -> dict:
    """This function extract the weather details for the coming week."""
    today = datetime.now()
...
def get_weather_details_for_24hrs(latitude: int, longitude: int) -> dict:
    ...
    response = requests.get(
        f'https://api.open-meteo.com/v1/forecast?latitude={str(latitude)}&longitude={str(longitude)}&hourly=cloud_cover,apparent_temperature,relative_humidity_2m,lightning_potential,precipitation,precipitation_probability,rain,snowfall,temperature_2m,uv_index,visibility,wind_direction_10m,wind_gusts_10m,wind_speed_10m,weather_code&start_hour={today_str}&end_hour={tomorrow_str}',
        timeout=3)
    return response.json()
...
def get_air_quality(latitude: float, longitude: float) -> dict:
    ...
    response = requests.get(
        api_url, headers={'X-Api-Key': ENV['API_KEY']}, timeout=3)
    return response.json()
